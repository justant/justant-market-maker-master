...
def trim_15m(bin5m_data):
    first_idx = 0
    last_idx = len(bin5m_data) - 1
    for i in range(0, len(bin5m_data)):
        temp_time = bin5m_data['timestamp'][i]
        dt = parse(temp_time)
        if dt.time().minute == 5 or dt.time().minute == 20 or dt.time().minute == 35 or dt.time().minute == 50:
            first_idx = i
            break

    for i in range(len(bin5m_data) - 1, len(bin5m_data) - 10, -1):
        temp_time = bin5m_data['timestamp'][i]
        dt = parse(temp_time)

        if dt.time().minute == 0 or dt.time().minute == 15 or dt.time().minute == 30 or dt.time().minute == 45:
            last_idx = i
            break
...
def get_analysis(getOnlyLast = False, bidSize = '1m'):
    ...
    analysis['stoch_k'], analysis['stoch_d'] = ta.STOCH(sec_id.high.to_numpy(), sec_id.low.to_numpy(), sec_id.close.to_numpy(), fastk_period=STOCH_K, slowk_period=STOCH_D, slowk_matype=0, slowd_period=3, slowd_matype=0) #slowk_period=self.STOCH_K, slowd_period=self.STOCH_D)
